...
SELECT id, workspace_id, daemon_id, name, runtime_mode, provider, status, device_info, metadata, last_seen_at, created_at, updated_at, owner_id, legacy_daemon_id, webhook_url, webhook_secret, webhook_event_type, timezone, visibility FROM agent_runtime
...
// of mode.
func (q *Queries) GetWebhookRuntimeConfig(ctx context.Context, id pgtype.UUID) (GetWebhookRuntimeConfigRow, error) {
	row := q.db.QueryRow(ctx, getWebhookRuntimeConfig, id)
	var i GetWebhookRuntimeConfigRow
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
		&i.WebhookUrl,
...
WHERE id = $1
RETURNING id, workspace_id, daemon_id, name, runtime_mode, provider, status, device_info, metadata, last_seen_at, created_at, updated_at, owner_id, legacy_daemon_id, webhook_url, webhook_secret, webhook_event_type, timezone, visibility
`
...
INSERT INTO agent_runtime (
    ...
    webhook_event_type,
    last_seen_at
) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, $13, $10, $11, $12, now())
...
DO UPDATE SET
    ...
    last_seen_at = now(),
    updated_at = now()
RETURNING id, workspace_id, daemon_id, name, runtime_mode, provider, status, device_info, metadata, last_seen_at, created_at, updated_at, owner_id, legacy_daemon_id, webhook_url, webhook_secret, webhook_event_type, timezone, visibility, (xmax = 0) AS inserted
`
