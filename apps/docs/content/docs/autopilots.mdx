...
- **Create issue mode** (`create_issue`) — default, **recommended**. Each trigger first creates an issue in the workspace, then assigns the issue to the agent through the normal assignment flow. All work lands on the issue board with the same history, comments, and status as a manually assigned issue.

  The title supports three placeholders; any other `{{...}}` token is rejected at create-time so a typo cannot silently land as the literal string in your issue titles:

  - `{{date}}` — interpolates to the UTC date in `YYYY-MM-DD` format.
  - `{{pr}}` — for a run triggered by a GitHub pull request webhook event, renders `owner/repo#123`. For a run with no pull request, falls back to the run's own id.
  - `{{pr_title}}` — for a PR-triggered run, renders the pull request's own title. Falls back to the autopilot's own title when there is no pull request — the same text `{{date}}`-only or empty templates already render today.

  A PR-triggered run also always leads its issue description with the identity line `owner/repo #123` (note the space before `#`, unlike `{{pr}}`), followed by the pull request's own summary, ahead of the autopilot's own description text.
